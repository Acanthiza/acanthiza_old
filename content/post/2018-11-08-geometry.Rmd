...
An eariler version of this post raised the question about trail vs. offset. Does one determine the other? Figure \@ref(fig:trailoff) shows the relationship between these two variables for each of the bikes in the assembled data set. There is no obvious relationship between the two. 

```{r trailoff, fig.cap = "Relationship between traill and (fork) offset (or for rake)"}

  ggplot(dat, aes(Trail,ForkRake, colour = Use)) +
    geom_point() +
    labs(x = "Trail (mm)"
         , y = "Rake (mm)"
         )
